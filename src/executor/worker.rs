...
use crate::executor::{Task,ExecutorMessage};
...
impl Worker {
    pub(crate) fn new(sender: Sender<ExecutorMessage>, receiver: Receiver<ExecutorMessage>) -> Worker {
        Worker{
    ...
    where F: Future<Output = ()> + 'static + Send {
        ...
        if let Err(PushError(task)) = self.local.push(task) {
            self.global_sender.send(ExecutorMessage::Task(task)).unwrap();
        }
    }
